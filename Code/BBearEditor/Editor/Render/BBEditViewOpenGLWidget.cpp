...
void BBEditViewOpenGLWidget::dragEnterEvent(QDragEnterEvent *event)
{
    ...
    if ((data = event->mimeData()->data(BB_MIMETYPE_BASEOBJECT)) != nullptr)
    {
        ...
        else if (m_DragType == BB_CLASSNAME_SPHFLUID)
        {
            m_pPreviewObject = m_pScene->createGameObject(event->pos().x(), event->pos().y(), BB_CLASSNAME_SPHFLUID);
        }
        else if (m_DragType == BB_CLASSNAME_CLOTH)
        {
