...
void BBScene::deferredRendering1_2()
{
    m_pCamera->switchTo3D();
    m_pCamera->update(m_fUpdateRate);
...
BBModel* BBScene::createModelForPreview(const QString &filePath, float fDistFactor)
{
    ...
    m_pCamera->update(position, viewCenter);
...
void BBScene::writeShadowMap()
{
    ...
    BBGlobalRenderState::updateCullState(true);
    BBGlobalRenderState::updateCullFace(GL_BACK);
