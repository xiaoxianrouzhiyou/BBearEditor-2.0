...
void main(void)
{
	...
	// Albedo = texture(ObjectDiffuseTexture, v2f_texcoord).rgb * max(dot(Normal.xyz, normalize(L)), 0) / dot(L, L) * 50.0;
    float intensity = max(dot(Normal.xyz, normalize(L)), 0.0) / dot(L, L) * 50.0;
