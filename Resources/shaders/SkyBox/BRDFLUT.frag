...
float computeGeometrySmith(vec3 N, vec3 V, vec3 L, float roughness)
{
    float NdotV = max(dot(N, V), 0.0);
    float NdotL = max(dot(N, L), 0.0);
    float ggx1 = computeGeometrySchlickGGX(NdotV, roughness);
    float ggx2 = computeGeometrySchlickGGX(NdotL, roughness);
...
vec2 computeIntegrateBRDF(float NdotV, float roughness)
{
    ...
    {
        // generates a sample vector that's based towards the preferred alignment direction (importance sampling).
        ...
        {
            // F is divided
            float G = computeGeometrySmith(N, V, L, roughness);
...
void main() 
{
    vec2 integrated_BRDF = computeIntegrateBRDF(v2f_texcoords.x, v2f_texcoords.y);
